...
- [x] Update the "Unreleased" section of CHANGELOG.md to add entries for the `make stats` Makefile target, the mutually-exclusive flag validation added to `scripts/todo_stats.py`'s argument parser, and the friendly missing-file error handling added to `scripts/check_todo_format.sh`, all of which were added in prior commits but are currently missing from the changelog
- [x] Fix `percent_complete()` in `scripts/todo_stats.py` so it rounds half away from zero instead of relying on Python's built-in `round()` (which uses banker's rounding), since e.g. 1 done out of 8 total items currently reports "12%" instead of the expected "13%"; add a regression test covering this case
- [ ] Add `--help`/`-h` usage output to `scripts/check_todo_format.sh` (e.g. a short usage message printed to stdout before exiting 0), mirroring the `--help`/`-h` flag already present in `scripts/todo_stats.py`, since running it with `-h` today just treats `-h` as the path argument and prints a "no such file" error
- [ ] Document the `make stats` target in CONTRIBUTING.md's "Running all checks" section, since it's currently only mentioned in README.md
- [ ] Add a test in `tests/test_check_todo_format.py` covering an empty checklist file (zero lines, or a file with only a heading and no checklist items), asserting `scripts/check_todo_format.sh` exits 0, since this edge case currently isn't covered
