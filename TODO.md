...
- [x] Update the "Unreleased" section of CHANGELOG.md to add entries for the `--version` flag in `scripts/todo_stats.py`, the `make clean` Makefile target, the optional path argument to `scripts/check_todo_format.sh`, and the new default-path test in `tests/test_todo_stats.py`, all of which were added in prior commits but are currently missing from the changelog
- [x] Fix `scripts/check_todo_format.sh` so it also checks the last line of the checklist file when that file doesn't end with a trailing newline (bash's `while read` loop currently skips an unterminated final line, silently letting a malformed last line pass), and add a regression test for this case
- [x] Add a test in `tests/test_check_todo_format.py` covering `scripts/check_todo_format.sh`'s default path argument (invoking it with no argument from the repo root and asserting it reads the real `TODO.md`), mirroring the equivalent default-path test already added for `todo_stats.py`'s `main()`
- [x] Add a return type hint (`-> argparse.ArgumentParser`) to `build_parser()` in `scripts/todo_stats.py`, since every other function in the module now has type hints except this one
- [x] Add a `--count`/`--total` flag to `scripts/todo_stats.py` that prints just the total number of checklist items (done + pending), useful for scripting against the backlog size without parsing the default human-readable output
- [x] Update the "Fixed" section of CHANGELOG.md to add an entry for the `scripts/check_todo_format.sh` fix that makes it also check the last line of the checklist file when that file doesn't end with a trailing newline, which was added in a prior commit but is currently undocumented in the changelog
- [ ] Update the stale example output for `scripts/todo_stats.py` shown in README.md's "Setup / usage" section (currently `22/24 tasks done (2 remaining)`), since it no longer matches the current TODO.md checklist totals
- [ ] Make `scripts/check_todo_format.sh` print a clear error message to stderr and exit non-zero when given a missing checklist file path, instead of letting bash's raw "No such file or directory" redirection error surface, mirroring the friendly missing-file handling already in `scripts/todo_stats.py`; add a regression test for this case
- [ ] Make the `--remaining`, `--percent`, and `--count`/`--total` flags mutually exclusive in `scripts/todo_stats.py`'s argument parser (e.g. via `argparse`'s `add_mutually_exclusive_group`), since passing more than one together currently silently honors only one flag instead of raising a usage error; add a test covering the new error
