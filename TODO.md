...
- [ ] Update the "Unreleased" section of CHANGELOG.md to add entries for the `make stats` Makefile target, the mutually-exclusive flag validation added to `scripts/todo_stats.py`'s argument parser, and the friendly missing-file error handling added to `scripts/check_todo_format.sh`, all of which were added in prior commits but are currently missing from the changelog
