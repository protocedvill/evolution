...
- [ ] Fix `percent_complete()` in `scripts/todo_stats.py` so it rounds half away from zero instead of relying on Python's built-in `round()` (which uses banker's rounding), since e.g. 1 done out of 8 total items currently reports "12%" instead of the expected "13%"; add a regression test covering this case
